...
class ChannelController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            ...
            'tier_id' => 'sometimes|exists:source_tiers,id'
        ]);

        $tier = DB::table('youtube_tiers_pivot')
            ->where('channel_id', $channel->channel_id)
            ->exists();

        DB::transaction(function() use ($channel, $validated, $tier) {
            $tierValue = isset($validated['tier_id']) ? $validated['tier_id'] : 3;

            $channel->update($validated);

            if($channel && $tier == true){
                DB::table('youtube_tiers_pivot')
                ->where('channel_id', $channel->channel_id)
                ->update(['tier_id' => $tierValue]);
            } elseif($channel && $tier == false){
